...
//
// Adapted from the https://github.com/OllieJones/h264-interp-utils package,
...
export type NaluStreamInfo = { type: StreamType; boxSize: number };

export type TypedNalu = { type: number; nalu: { rawNalu: Uint8Array; nalu: Uint8Array } };
...
export class RawBitstream {
  ...
  seek(pos = 0) {
    if (pos > this.max) {
      throw new Error("cannot seek beyond end");
    }
    this.ptr = pos;
  }

  reallocate(size: number) {
    if (this.ptr + size <= this.max) {
      return;
    }
...
export class Bitstream extends RawBitstream {
  ...
  override copyBits(from: Bitstream, ptr: number, count: number, to: number) {
  ...
  deemulate(stream: Uint8Array) {
    ...
    while (p < len) {
      if (stream[p - 2] === 0 && stream[p - 1] === 0 && stream[p] === 3 && stream[p]! <= 3) {
        p++;
      } else {
        buf[q++] = stream[p++]!;
      }
...
export class NALUStream {
  ...
  static readUIntNBE(buff: Uint8Array, ptr: number, boxSize: number) {
    if (!boxSize) {
      throw new Error("readUIntNBE error: need a boxsize");
    }
    let result = 0 | 0;
  ...
  convertToPacket() {
    ...
    if (this.type === "annexB" && this.boxSize === 4) {
      this.iterate((buff, first, last) => {
        let p = first - 4;
        if (p < 0) {
          throw new Error("NALUStream error: Unexpected packet format");
        }
      ...
      this.iterate((buff, first, last) => {
        let p = first - 3;
        if (p < 0) {
          throw new Error("Unexpected packet format");
        }
        const len = last - first;
    ...
    while (true) {
      if (delim.e > delim.s) {
        ...
        if (typeof callback === "function") {
          callback(this.buf, delim.s, delim.e);
        }
      }
      if (delim.n < 0) {
        break;
  ...
  getType = (scanLimit: number): { boxSize: number; type: StreamType } => {
    ...
    for (let boxSize = 4; boxSize >= 1; boxSize--) {
      ...
      while (true) {
        ...
        if (delim.e - delim.s) {
          packetCount++;
          if (scanLimit && packetCount >= scanLimit) {
            break;
          }
        }
        if (delim.n < 0) {
      ...
      if (packetCount > 0) {
        return { type: "packet", boxSize };
      }
    }
    if (this.strict) {
      throw new Error("NALUStream error: cannot determine stream type or box size");
    }
...
export class SPS {
  ...
  constructor(SPS: Uint8Array) {
    ...
    if (this.nal_unit_type !== 7) {
      throw new Error(`SPS error: not SPS (nal_unit_type=${this.nal_unit_type})`);
    }

    this.profile_idc = bitstream.u_8()!;
    const profileName = profileNames.get(this.profile_idc);
    if (profileName != undefined) {
      this.profileName = profileName;
    } else {
      throw new Error(`SPS error: invalid profile_idc ${this.profile_idc}`);
    }
